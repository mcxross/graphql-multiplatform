 ...
 *
 * This is a workaround to kotlinx.serialization limitation that cannot process arbitrary maps
 * (required for handling extensions in GraphQL responses) as it relies on compiler to generate
 * appropriate serializers and cannot determine appropriate one to process generic Any object at
 * runtime.
 *
 * During serialization AnyKSerializer relies on Kotlin reflections to find all object member
 * properties.
 *
 * JsonDecoder always represents underlying primitive JSON elements as String with additional flag
 * specifying whether it was a String or not. This means that we have to explicitly attempt to
  ...
  private fun serializeAny(value: Any?): JsonElement? =
    when (value) {
      ...
      is Map<*, *> -> {
        val mapContents =
          value
            .mapNotNull { (key, value) -> serializeAny(value)?.let { key.toString() to it } }
            .toMap()
      ...
      is String -> JsonPrimitive(value)
        ...
        JsonObject(contents)*/
        // TODO: This is a workaround to avoid reflection, we should find a better way to serialize
        // Any
        JsonObject(emptyMap())
      }
    }
