...
package xyz.mcxross.graphql.plugin.gradle.client.generator
...
private const val CORE_TYPES_PACKAGE = "xyz.mcxross.graphql.client.types"

/**
 * GraphQL client code generator that uses [KotlinPoet](https://github.com/square/kotlinpoet) to
 * generate Kotlin classes based on the specified GraphQL queries.
 */
class GraphQLClientGenerator(schemaPath: String, private val config: GraphQLClientGeneratorConfig) {
  ...
  fun generate(queries: List<File>): List<FileSpec> {
    ...
    if (typeAliases.isNotEmpty()) {
      val typeAliasSpec =
        FileSpec.builder(packageName = config.packageName, fileName = "GraphQLTypeAliases")
      typeAliases.forEach { (_, alias) -> typeAliasSpec.addTypeAlias(alias) }
  ...
  internal fun generate(queryFile: File): List<FileSpec> {
    ...
    val operationFileSpec =
      FileSpec.builder(
        packageName = config.packageName,
        fileName = queryFile.nameWithoutExtension.capitalizeFirstChar(),
      )
    operationDefinitions.forEach { operationDefinition ->
      val capitalizedOperationName =
        operationDefinition.name?.capitalizeFirstChar()
          ?: queryFile.nameWithoutExtension.capitalizeFirstChar()
      val context =
      ...
      val graphQLResponseTypeSpec =
        generateGraphQLObjectTypeSpec(context, rootType, operationDefinition.selectionSet, "Result")
      val kotlinResultTypeName =
        ClassName(context.packageName, "${context.operationName}.${graphQLResponseTypeSpec.name}")
      ...
      var queryProperty: PropertySpec =
      ...
      val variableType: TypeSpec? =
        generateVariableTypeSpec(context, operationDefinition.variableDefinitions)
      if (variableType != null) {
        operationTypeSpec.addType(variableType)

        val variablesClassName =
          ClassName(config.packageName, "$capitalizedOperationName.Variables")
        val variablesProperty =
          PropertySpec.builder("variables", variablesClassName, KModifier.OVERRIDE)
      ...
      for ((superClassName, implementations) in context.polymorphicTypes) {
        ...
        val polymorphicTypeSpec =
          FileSpec.builder(superClassName.packageName, superClassName.simpleName)
      ...
      context.scalarClassToConverterTypeSpecs.values.forEach {
        when (it) {
  ...
  private fun findRootType(operationDefinition: OperationDefinition): ObjectTypeDefinition {
    val operationNames =
      if (graphQLSchema.schemaDefinition().isPresent) {
        graphQLSchema
          .schemaDefinition()
          .get()
          .operationTypeDefinitions
          .associateBy({ it.name.uppercase() }, { it.typeName.name })
      } else {
        mapOf(
    ...
    return if (schemaFile.isFile) {
      ...
      val schemaInputStream =
        this.javaClass.classLoader.getResourceAsStream(path)
          ?: throw SchemaUnavailableException(path)
