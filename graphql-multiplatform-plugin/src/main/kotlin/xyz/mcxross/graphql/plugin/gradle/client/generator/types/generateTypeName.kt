...
/**
 * Generate custom [ClassName] reference to a Kotlin class representing GraphQL complex type
 * (object, input object, enum, interface, union or custom scalar). If class name was not yet
 * generated, this method will generate the underlying type spec definition as well. If class name
 * was retrieved from the cache we re-validate interface and object types to ensure they were
 * generated with the same selection set.
 */
internal fun generateCustomClassName(
  context: GraphQLClientGeneratorContext,
  graphQLType: NamedNode<*>,
  selectionSet: SelectionSet? = null,
): ClassName {
  val graphQLTypeDefinition: TypeDefinition<*> =
    context.graphQLSchema.getType(graphQLType.name).get()
  ...
  return if (cachedTypeNames == null || cachedTypeNames.isEmpty()) {
    ...
    if (
      ...
      when (graphQLTypeDefinition) {
        is ObjectTypeDefinition -> {
          ...
          context.typeSpecs[className] =
            generateGraphQLObjectTypeSpec(context, graphQLTypeDefinition, selectionSet)
        }
        is InputObjectTypeDefinition -> {
          className =
            generateClassName(
              ...
              selectionSet,
              packageName = "${context.packageName}.inputs",
            )
          context.inputClassToTypeSpecs[className] =
            generateGraphQLInputObjectTypeSpec(context, graphQLTypeDefinition)
        }
        is EnumTypeDefinition -> {
          className =
            generateClassName(
              ...
              graphQLTypeDefinition,
              selectionSet,
              packageName = "${context.packageName}.enums",
            )
          context.enumClassToTypeSpecs[className] =
        ...
        is InterfaceTypeDefinition -> {
          ...
          context.typeSpecs[className] =
            generateGraphQLInterfaceTypeSpec(context, graphQLTypeDefinition, selectionSet)
        }
        is UnionTypeDefinition -> {
          className = generateClassName(context, graphQLTypeDefinition, selectionSet)
          context.polymorphicTypes[className] = mutableListOf(className)
          context.typeSpecs[className] =
            generateGraphQLUnionTypeSpec(context, graphQLTypeDefinition, selectionSet)
        }
        is ScalarTypeDefinition -> {
          ...
          val converterInfo =
            generateGraphQLCustomScalarConverters(
              context,
              className,
              graphQLScalarMapping.converterClassName,
            )
    ...
    // verify we got same selection set for interface and/or objects (unions shouldn't have any
    // fields)
    for (cachedType in cachedTypeNames) {
      if (
        isCachedTypeApplicable(
          context,
          cachedType.simpleNameWithoutWrapper(),
          graphQLTypeDefinition,
          selectionSet,
        )
      ) {
...
/**
 * Generate custom [ClassName] reference to a Kotlin class representing GraphQL complex type
 * (object, input object, enum, interface, union or custom scalar) and caches the value.
 */
...
private fun isCachedTypeApplicable(
  context: GraphQLClientGeneratorContext,
...
private fun verifySelectionSet(
  context: GraphQLClientGeneratorContext,
  graphQLTypeName: String,
  selectionSet: SelectionSet,
): Boolean {
    ...
    when (selection) {
      is Field -> {
        ...
        if (selection.selectionSet != null) {
          result.addAll(
            calculateSelectedFields(context, targetType, selection.selectionSet, "$path$fieldName.")
          )
        }
      }
      is InlineFragment -> {
        val targetFragmentType = selection.typeCondition.name
        val fragmentPathPrefix =
          if (targetFragmentType == targetType) {
            path
          } else {
            "$path$targetFragmentType."
          }
        result.addAll(
          calculateSelectedFields(context, targetType, selection.selectionSet, fragmentPathPrefix)
        )
      }
      is FragmentSpread -> {
        val fragmentDefinition =
          context.queryDocument.findFragmentDefinition(context, selection.name, targetType)
