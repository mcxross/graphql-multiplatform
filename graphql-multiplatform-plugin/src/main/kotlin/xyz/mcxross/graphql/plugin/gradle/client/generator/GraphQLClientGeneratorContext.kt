 ...
 *
 * Context is created per each processed GraphQL query and contains configuration information,
 * schema and query information as well as holds caches of all generated types, aliases and class
 * names.
 */
data class GraphQLClientGeneratorContext(
  ...
  private val customScalarClassNames: Set<ClassName> =
    customScalarMap.values.map { it.className }.toSet()

  internal fun isCustomScalar(typeName: TypeName): Boolean =
    customScalarClassNames.contains(typeName)
