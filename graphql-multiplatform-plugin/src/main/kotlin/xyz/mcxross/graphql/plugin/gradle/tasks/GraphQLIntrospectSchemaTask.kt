...
import xyz.mcxross.graphql.plugin.gradle.actions.IntrospectSchemaAction
...
/**
 * Task that executes GraphQL introspection query against specified endpoint and saves the
 * underlying schema file.
 */
abstract class GraphQLIntrospectSchemaTask : DefaultTask() {
  ...
  /**
   * Timeout configuration that specifies maximum amount of time (in milliseconds) to connect and
   * execute introspection query before we cancel the request. Defaults to Ktor CIO engine defaults
   * (5 seconds for connect timeout and 15 seconds for read timeout).
   */
  @Input
  val timeoutConfig: Property<TimeoutConfiguration> =
    project.objects.property(TimeoutConfiguration::class.java)

  /** Boolean property to indicate whether to use streamed (chunked) responses. */
  @Input val streamResponse: Property<Boolean> = project.objects.property(Boolean::class.java)
  ...
  /**
   * Executes introspection query against specified endpoint and saves the resulting schema locally
   * in the target output file.
   */
  @TaskAction
