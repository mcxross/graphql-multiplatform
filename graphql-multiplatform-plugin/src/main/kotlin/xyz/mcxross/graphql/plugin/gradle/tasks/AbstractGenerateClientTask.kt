...
abstract class AbstractGenerateClientTask : DefaultTask() {
  ...
  @get:Classpath val pluginClasspath: ConfigurableFileCollection = project.objects.fileCollection()
   ...
   * **Required Property**
   */
  @InputFile val schemaFile: RegularFileProperty = project.objects.fileProperty()

  /**
   * Target package name for generated code.
   *
   * **Required Property** **Command line property is**: `packageName`.
   */
  @Input
   ...
   *
   * **Default value is:** `false`. **Command line property is**: `allowDeprecatedFields`.
   */
  @Input
  @Optional
  @Option(
    option = "allowDeprecatedFields",
    description = "boolean flag indicating whether selection of deprecated fields is allowed or not",
  )
  ...
  val customScalars: ListProperty<GraphQLScalar> =
    project.objects.listProperty(GraphQLScalar::class.java)

  /**
   * Directory containing GraphQL queries. Defaults to `src/main/resources` when generating main
   * sources and `src/test/resources` when generating test client.
   *
   * Instead of specifying a directory you can also specify list of query file by using `queryFiles`
   * property instead.
   */
  @InputDirectory
  @Optional
  ...
  /**
   * List of query files to be processed. Instead of a list of files to be processed you can also
   * specify [queryFileDirectory] directory containing all the files. If this property is specified
   * it will take precedence over the corresponding directory property.
   */
  @InputFiles
  @Optional
  ...
  val parserOptions: Property<GraphQLParserOptions> =
    project.objects.property(GraphQLParserOptions::class.java)
  ...
  fun generateGraphQLClientAction() {
    ...
    val targetQueryFiles: List<File> =
      when {
        queryFiles.files.isNotEmpty() -> queryFiles.files.toList()
        queryFileDirectory.isPresent -> {
          queryFileDirectory.asFile.orNull
            ?.walkBottomUp()
            ?.filter { file -> file.extension == "graphql" }
    ...
    val workQueue: WorkQueue =
      getWorkerExecutor().classLoaderIsolation { workerSpec: ClassLoaderWorkerSpec ->
