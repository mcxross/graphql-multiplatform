...
/**
 * Generate enum [TypeSpec] from the specified GraphQL enum definition. Adds default
 * `__UNKNOWN_VALUE` relying on Jackson annotation to handle new/unknown enum values.
 */
internal fun generateGraphQLEnumTypeSpec(
  context: GraphQLClientGeneratorContext,
  enumDefinition: EnumTypeDefinition,
): TypeSpec {
  val enumTypeSpecBuilder =
    TypeSpec.enumBuilder(enumDefinition.name).addAnnotation(Generated::class)
  enumDefinition.description?.content?.let { kdoc -> enumTypeSpecBuilder.addKdoc("%L", kdoc) }
  enumDefinition.enumValueDefinitions.forEach { enumValueDefinition ->
    ...
    val deprecatedDirective =
      enumValueDefinition.getDirectives(DeprecatedDirective.name).firstOrNull()
