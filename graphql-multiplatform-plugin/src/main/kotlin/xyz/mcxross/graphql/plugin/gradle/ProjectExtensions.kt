...
internal fun configureAndroidCompileTasks(
  project: Project,
  clientGeneratingTasks: List<GraphQLGenerateClientTask>,
  testClientGeneratingTasks: List<GraphQLGenerateTestClientTask>,
) {
  val androidExtension = project.extensions.findByType(BaseExtension::class.java)

  val (variantCompileNames, testVariantCompileNames) =
    when (androidExtension) {
      is LibraryExtension ->
        androidExtension.libraryVariants.map { it.calculateCompileTaskName() } to
          findTestVariants(androidExtension).map { it.calculateCompileTaskName() }
      is AppExtension ->
        androidExtension.applicationVariants.map { it.calculateCompileTaskName() } to
          findTestVariants(androidExtension).map { it.calculateCompileTaskName() }
      else ->
