...
import xyz.mcxross.graphql.plugin.gradle.actions.DownloadSDLAction
...
/**
 * Task that attempts to download GraphQL schema in SDL format from the specified endpoint and save
 * it locally.
 */
...
abstract class GraphQLDownloadSDLTask : DefaultTask() {
   ...
   * download schema from SDL endpoint before we cancel the request. Defaults to Ktor CIO engine
   * defaults (5 seconds for connect timeout and 15 seconds for read timeout).
   */
  @Input
  ...
  /**
   * Download schema in SDL format from the specified endpoint and save it locally in the target
   * output file.
   */
  @TaskAction
