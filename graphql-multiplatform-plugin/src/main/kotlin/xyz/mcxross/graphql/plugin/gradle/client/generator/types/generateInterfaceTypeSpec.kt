...
/**
 * Generate interface [TypeSpec] based on the available field definitions and selection set.
 * Generates all implementing classes as well.
 *
 ...
 * - all polymorphic types need to have consistent selection set within the query (i.e. you cannot
 *   select same polymorphic type with different selection sets in single query).
 *
...
internal fun generateInterfaceTypeSpec(
  ...
  val namedFragments =
    selectionSet
      .getSelectionsOfType(FragmentSpread::class.java)
      .map { fragment ->
        // polymorphic selection set can contain selection set against interface or concrete types
        context.queryDocument.getDefinitionsOfType(FragmentDefinition::class.java).find {
          it.name == fragment.name
        } ?: throw InvalidFragmentException(context.operationName, fragment.name, interfaceName)
      }
  ...
  val selections: MutableList<Selection<*>> =
    selectionSet.getSelectionsOfType(Field::class.java).toMutableList()

  // interface fields
  namedFragments[interfaceName]?.let { selections.addAll(it.selectionSet.selections) }
  ...
  val implementationSelections =
    namedFragments
      .filterKeys { it != interfaceName }
  ...
  selectionSet.getSelectionsOfType(InlineFragment::class.java).forEach { fragment ->
    val existing =
      implementationSelections.computeIfAbsent(fragment.typeCondition.name) {
  ...
  if (notImplemented.isNotEmpty()) {
    logger.warn(
      "Operation ${context.operationName} does not specify all polymorphic implementations - field selection on $interfaceName is missing $notImplemented"
    )
  }
  ...
  implementationSelections.forEach { implementationName, (typeCondition, selections) ->
    val distinctSelections =
      selections.filterIsInstance(Field::class.java).distinctBy {
        if (it.alias != null) it.alias else it.name
      }
    ...
    var implementationClassName =
      generateTypeName(
        context,
        typeCondition,
        SelectionSet.newSelectionSet(distinctSelections).build(),
      )
    if (implementationClassName.isNullable) {
      implementationClassName = implementationClassName.copy(nullable = false)
    }
    updateImplementationTypeSpecWithSuperInformation(
      context,
      interfaceName,
      implementationName,
      implementationClassName as ClassName,
      commonProperties,
    )
    ...
    jsonSubTypesCodeBlock.add(
      "com.fasterxml.jackson.annotation.JsonSubTypes.Type(value = %T::class, name=%S)",
      unwrappedClassName,
      implementationName,
    )
  }
...
private fun verifyTypeNameIsSelected(selections: List<Selection<*>>) =
  selections.filterIsInstance(Field::class.java).any { it.name == "__typename" }
...
private fun generateFallbackImplementation(
  context: GraphQLClientGeneratorContext,
  interfaceName: String,
  commonProperties: List<PropertySpec>,
): ClassName {
  ...
  val superClassName =
    ClassName("${context.packageName}.${context.operationName.lowercase()}", interfaceName)
  val fallbackClassName =
    ClassName("${context.packageName}.${context.operationName.lowercase()}", fallbackTypeName)
  val fallbackType =
    TypeSpec.classBuilder(fallbackTypeName)
      .addAnnotation(Generated::class)
      .addKdoc(
      ...
      .addProperties(
        commonProperties.map { abstractProperty ->
          abstractProperty
            ...
            .also { it.modifiers.remove(KModifier.ABSTRACT) }
            .build()
        }
      )
      .primaryConstructor(
          ...
          .addParameters(
            commonProperties.map { propertySpec ->
              ...
              if (propertySpec.type.isNullable) {
                ...
                className != null && context.enumClassToTypeSpecs.keys.contains(className)
              ) {
