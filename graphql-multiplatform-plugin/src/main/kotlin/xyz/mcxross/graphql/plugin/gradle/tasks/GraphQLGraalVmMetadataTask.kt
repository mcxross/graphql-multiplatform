...
abstract class GraphQLGraalVmMetadataTask : SourceTask() {
  ...
  @get:Classpath val projectClasspath: ConfigurableFileCollection = project.objects.fileCollection()

  /** List of supported packages that can contain GraphQL schema type definitions. */
  @Input val packages: ListProperty<String> = project.objects.listProperty(String::class.java)
  ...
  init {
    ...
    outputDirectory.convention(
      project.layout.buildDirectory.dir("generated/graphqlGraalVmResources")
    )
  }
  ...
  fun generateMetadata() {
    ...
    if (packages.isEmpty()) {
      throw RuntimeException(
        "attempt to generate SDL failed - missing required supportedPackages property"
      )
    }

    val targetDirectory =
      outputDirectory
        .dir("META-INF/native-image/${project.group}/${project.name}/graphql")
        .get()
        .asFile
    ...
    logger.debug(
      "submitting work item to generate GraalVM for the schema included in packages = {}",
      packages,
    )
